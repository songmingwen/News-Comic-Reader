...
public abstract class BaseVideoController
    ...
    public void setNormalVideoPlayer(INormalVideoPlayer normalVideoPlayer) {
        mNormalVideoPlayer = normalVideoPlayer;
    }
     ...
     *                  <ul>
     *                  <li>{@link NormalVideoPlayer#STATE_IDLE}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link NormalVideoPlayer#STATE_ERROR}</li>
     ...
     *                 <ul>
     *                 <li>{@link NormalVideoPlayer#MODE_NORMAL}</li>
     *                 <li>{@link NormalVideoPlayer#MODE_FULL_SCREEN}</li>
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (mNormalVideoPlayer.isIdle()
                || mNormalVideoPlayer.isError()
                || mNormalVideoPlayer.isPreparing()
                || mNormalVideoPlayer.isPrepared()
                || mNormalVideoPlayer.isCompleted()) {
            hideChangePosition();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        mGestureDownPosition = mNormalVideoPlayer.getCurrentPosition();
                    } else if (absDeltaY >= THRESHOLD) {
                        if (mDownX < getWidth() * 0.5f) {
                            // 左侧改变亮度
                            mNeedChangeBrightness = true;
                            mGestureDownBrightness = ScreenUtils.scanForActivity(mContext)
                                    .getWindow().getAttributes().screenBrightness;
                        } else {
                            // 右侧改变声音
                            mNeedChangeVolume = true;
                            mGestureDownVolume = mNormalVideoPlayer.getVolume();
                        }
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                if (mNeedChangePosition) {
                    mNormalVideoPlayer.seekTo(mNewPosition);
                    hideChangePosition();
