...
public class ARToolKit {
    ...
    static {
        ...
        if (!loadedNative) ArLog.e(TAG, "Loading native library failed!");
        else ArLog.i(TAG, "Loaded native library.");
    }
     ...
     */
    private ARToolKit() {
        ArLog.i(TAG, "ARToolKit(): ARToolKit constructor");
    }
    ...
    public boolean initialiseNative(String resourcesDirectoryPath) {
        ...
        if (!NativeInterface.arwInitialiseAR()) {
            ArLog.e(TAG, "initialiseNative(): Error initialising native library!");
            return false;
        }
        ArLog.i(TAG, "initialiseNative(): ARToolKit version: " + NativeInterface.arwGetARToolKitVersion());
        if (!NativeInterface.arwChangeToResourcesDir(resourcesDirectoryPath)) {
            ArLog.i(TAG, "initialiseNative(): Error while attempting to change working directory to resources directory.");
        }
        initedNative = true;
        return true;
    }
	...
	public boolean initialiseNativeWithOptions(String resourcesDirectoryPath, int pattSize, int pattCountMax) {
		...
		if (!NativeInterface.arwInitialiseARWithOptions(pattSize, pattCountMax)) {
			ArLog.e(TAG, "Error initialising native library!");
			return false;
		}
		ArLog.i(TAG, "ARToolKit version: " + NativeInterface.arwGetARToolKitVersion());
		if (!NativeInterface.arwChangeToResourcesDir(resourcesDirectoryPath)) {
			ArLog.i(TAG, "Error while attempting to change working directory to resources directory.");
		}
		initedNative = true;
		return true;
	}
    ...
    public boolean startWithPushedVideo(int videoWidth, int videoHeight, String pixelFormat, String cameraParaPath, int cameraIndex, boolean cameraIsFrontFacing) {
        ...
        if (!initedNative) {
            ArLog.e(TAG, "startWithPushedVideo(): Cannot start because native interface not inited.");
            return false;
        }

        if (!NativeInterface.arwStartRunning("", cameraParaPath, 10.0f, 10000.0f)) {
            ArLog.e(TAG, "startWithPushedVideo(): Error starting");
            return false;
        }
        if (0 > NativeInterface.arwAndroidVideoPushInit(0, videoWidth, videoHeight,
                                                        pixelFormat, cameraIndex,
                                                        (cameraIsFrontFacing ? 1 : 0))) {
            ArLog.e(TAG, "startWithPushedVideo(): Error initialising Android video");
            return false;
        }
    ...
    public float[] calculateReferenceMatrix(int idMarkerBase, int idMarker2) {
        ...
        if (referenceMarkerTranslationMatrix != null && secondMarkerTranslationMatrix != null) {
            ...
            ArLog.e(TAG, "calculateReferenceMatrix(): Currently there are no two markers visible at the same time");
            return null;
        }
    }
    ...
    public float distance(int referenceMarker, int markerId2) {
        ...
        if (referenceMatrix != null) {
            ...
            ArLog.d(TAG, "distance(): Marker distance: x: " + distanceX + " y: " + distanceY + " z: " + distanceZ);
