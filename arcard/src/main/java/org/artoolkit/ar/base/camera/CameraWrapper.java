...
class CameraWrapper {
    ...
    public CameraWrapper(Camera cam) {
        ...
        try {
            ...
            ArLog.i(TAG, "CameraWrapper(): Found class " + CAMERA_CLASS_NAME);

            setPreviewCallbackMethod = cameraClass.getDeclaredMethod("setPreviewCallback", new Class[]{Camera.PreviewCallback.class});
            ArLog.i(TAG, "CameraWrapper(): Found method setPreviewCallback");
            ...
            ArLog.i(TAG, "CameraWrapper(): Found method setPreviewCallbackWithBuffer");

            addCallbackBufferMethod = cameraClass.getDeclaredMethod("addCallbackBuffer", new Class[]{byte[].class});
            ArLog.i(TAG, "CameraWrapper(): Found method addCallbackBuffer");

        } catch (NoSuchMethodException nsme) {
            ArLog.w(TAG, "CameraWrapper(): Could not find method: " + nsme.getMessage());


        } catch (ClassNotFoundException cnfe) {
            ArLog.w(TAG, "CameraWrapper(): Could not find class " + CAMERA_CLASS_NAME);
        }
    ...
    public boolean configureCallback(Camera.PreviewCallback cb, boolean useBuffersIfAvailable, int numBuffersIfAvailable, int bufferSize) {
        ...
        if (success) {
            ...
            if (usingBuffers) {
                ArLog.i(TAG, "configureCallback(): Configured camera callback using " + numBuffersIfAvailable + " buffers of " + bufferSize + " bytes");
            } else {
                ArLog.i(TAG, "configureCallback(): Configured camera callback without buffers");
            }
    ...
    private boolean addCallbackBuffer(byte[] data) {
        ...
        try {
            ...
            //ArLog.d(TAG, "Returned camera data buffer to pool");
